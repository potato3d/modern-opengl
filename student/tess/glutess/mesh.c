...
**
** $Date$ $Revision$
** $Header: //depot/main/gfx/lib/eng/libtess/mesh.c#6 $
*/
 ...
 * For more explanation see __tess_meshSplice() below.
 */
...
/* __tess_meshMakeEdge creates one edge, two vertices, and a loop (face).
 ...
 *
 * This function could be implemented as two calls to __tess_meshSplice
...
{
  TESShalfEdge *eNew;
  TESShalfEdge *tempHalfEdge= __tess_meshAddEdgeVertex( eOrg );
...
void __tess_meshZapFace( TESSface *fZap )
{
  ...
  do {
    ...
    e->Lface = NULL;
    if( e->Rface == NULL ) {
      /* delete the edge -- see __tess_MeshDelete above */
