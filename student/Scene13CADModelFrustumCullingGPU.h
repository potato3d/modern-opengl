...
public:
	bool initialize()
	{
		...
		std::string basepath = "C:/Users/admin/Downloads/";
		...
		//scene13: compute the number of groups to be dispatched according to scene size and block size
		// each shader invocation will compute a single geometry

		//scene13: tell compute shader how many geometries are in the scene, so any extra shader invocations can return immediatelly

		// -------------------------------------------------------------------------------------------
		// 9- Setup atomic counter to keep track of how many draw calls were generated inside the GPU
		// -------------------------------------------------------------------------------------------

		//scene13: create buffer
	...
	void draw(const CameraData& cameraData)
	{
		...
		//scene13: update frustum data and send to GPU

		//scene13: clear atomic counter (zero how many draw calls were generated in the previous frame)

		//scene13: clear draw command buffer (maybe it is more efficient to use a compute shader or to copy from another gpu buffer)
		// we take benefit of the fact that if data is null, the range is filled with zeroes

		// bind stuff to compute
		//scene13: use program
		//scene13: bind atomic counter
		//scene13: bind draw cmds (input)
		//scene13: bind visible draw cmds (output)
		//scene13: bind bounds
		//scene13: bind frustum

		//scene13: dispatch compute

		//scene13: insert memory barrier to guarantee data will be visible when drawing
		...
		// bind stuff to draw
		//scene13: bind program
		//scene13: bind vao
		//scene13: bind transforms
		//scene13: bind materials
		//scene13: bind visible draw cmds

		//scene13: draw indirect using commands generated by compute shader inside GPU
		...
		// remember to comment the old draw call above
