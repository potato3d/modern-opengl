...
public:
	bool initialize()
	{
		...
		// get bindless texture handles from the combination of texture + sampler
		_bindlessTextureHandles.reserve(_textures.size());
		for(auto t : _textures)
		{
			_bindlessTextureHandles.push_back(glGetTextureSamplerHandleARB(t, _textureSampler));
			glMakeTextureHandleResidentARB(_bindlessTextureHandles.back());
		}

		// create SSBO to store texture handles
		glCreateBuffers(1, &_bindlessTextureSSBO);
		glNamedBufferStorage(_bindlessTextureSSBO, _bindlessTextureHandles.size()*sizeof(GLuint64), _bindlessTextureHandles.data(), 0); // flags = 0
	...
	void draw(const CameraData& cameraData)
	{
		...
		glBindBufferBase(GL_SHADER_STORAGE_BUFFER, SB_TEXTURE, _bindlessTextureSSBO);
