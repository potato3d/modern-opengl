...
public:
	bool initialize()
	{
		...
		for(unsigned int z = 0; z < numZ; ++z)
		{
			for(unsigned int y = 0; y < numY; ++y)
			{
				for(unsigned int x = 0; x < numX; ++x)
				{
					...
					drawCmd.baseInstance = _drawCmds.size(); // automatically fetch the drawID instanced attribute
		...
		glCreateBuffers(1, &_drawCmdsBuffer);
		glNamedBufferStorage(_drawCmdsBuffer, _drawCmds.size()*sizeof(DrawCommand), _drawCmds.data(), 0); // flags = 0

		// ------------------------------------------------------------------------
		// 7- Setup custom draw ID
		// ------------------------------------------------------------------------

		std::vector<int> drawIDs(_drawCmds.size());
		...
		glVertexArrayVertexBuffer(_vao, bufferIndex, drawIdBuffer, 0, sizeof(int)); // offset = 0, stride = sizeof(int)
		...
		// setup drawID attrib
		glEnableVertexArrayAttrib(_vao, IN_DRAWID);
		glVertexArrayAttribBinding(_vao, IN_DRAWID, bufferIndex);
		glVertexArrayAttribIFormat(_vao, IN_DRAWID, 1, GL_INT, 0); // size = 1, offset = 0
	...
	void draw(const CameraData& cameraData)
	{
		...
		glBindBuffer(GL_DRAW_INDIRECT_BUFFER, _drawCmdsBuffer);
		glMultiDrawElementsIndirect(GL_TRIANGLES, GL_UNSIGNED_INT, 0, _sceneSize, 0); // offset = 0, stride = 0
	}
